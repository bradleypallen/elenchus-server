...
- `ELENCHUS_MAX_OPEN_BASES` / `ELENCHUS_BASE_IDLE_SECONDS` — how many
  ...
  open base is ~10 MB, so size the bound to the box's memory.
...
4. **db/registry.py** — Process-wide owner of all DuckDB connections. Holds the platform DB open for the lifetime of the server; per-base files are loaded lazily into a **bounded LRU** (`BaseHandle` per name, each with an `asyncio.Lock` for write serialization) and **closed again when nobody is using them**: each open base costs ~10 MB, so a sweep — after every open, on every `hold()` release, and every `SWEEP_INTERVAL_SECONDS` from the lifespan task — closes handles idle for `ELENCHUS_BASE_IDLE_SECONDS` (default 900) and, above `ELENCHUS_MAX_OPEN_BASES` (default 32), the least recently used, **never** one whose lock is taken, whose pin count is above zero, or that was looked up within `EVICT_GRACE_SECONDS` (a sync route may still be using it). Code that keeps a state across a long wait *without* the lock must pin it with `with reg.hold(name) as handle:` (the message route and report generation do, across the LLM call); a one-off read of a base (backup, export, integrity) uses `hold(name, transient=True)` so the base is closed on release instead of staying warm. Closing happens under the registry lock, through the one close path, so a reopen can't overlap a close. `db_path(name)` resolves through `platform.bases` to `bases/{owner_id}/{name}.duckdb`. Single-writer-per-file is a hard constraint of DuckDB; the server runs as one process. Migration to Postgres swaps this module out.
