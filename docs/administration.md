...
| `ELENCHUS_LOG_LEVEL` | how much the server writes to its log: `INFO` records every LLM call, every alert, every change to the ledger and to study settings; `WARNING` only problems | `INFO` |
